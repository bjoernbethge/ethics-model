import torch
from ethics_model.modules.attention import EthicalAttention, GraphAttentionLayer

def test_ethical_attention_forward(summary_writer, cpu_or_cuda_profiler, symbolic_constraints):
    model = EthicalAttention(d_model=16, n_heads=2)
    x = torch.randn(2, 5, 16)
    out, attn = model(x, x, x, symbolic_constraints=symbolic_constraints)
