...
def test_ethics_model_forward(summary_writer, cpu_or_cuda_profiler, symbolic_constraints):
    model = EthicsModel(input_dim=16, d_model=16, n_layers=2, n_heads=2, vocab_size=100, max_seq_length=10, use_gnn=False)
    input_ids = torch.randint(0, 100, (2, 5))
    attention_mask = torch.ones(2, 5)
    outputs = model(input_ids, attention_mask, symbolic_constraints=symbolic_constraints)
